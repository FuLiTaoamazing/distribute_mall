package com.flt.gateway.filter;
...
@Configuration
//ConditionOnProperty 通过条件来进行控制配置的生效与否,
@ConditionalOnProperty(value = "log.request.enable",havingValue = "true",matchIfMissing = true)
public class RequestLogFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 打印执行时间
        log.info(reqLog.toString(),reqArgs);
    ...
    public String getOriginalRequestUrl(ServerWebExchange exchange){
        ServerHttpRequest request = exchange.getRequest();
        ...
        return UriComponentsBuilder.fromPath(requestUri.getPath()).queryParams(queryParams).build().toUriString();
    }
}
